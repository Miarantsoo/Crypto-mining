...
const Profil: React.FC = () => {
  ...
  if (!user) {
    return <p className="text-center text-lg text-gray-500">Chargement du profil...</p>;
  }

  return (
      <div className="flex flex-row h-full">
        <div className="flex flex-row gap-5 w-5/6">
          <img src={Pfp} alt="Photo de profil" className="w-1/3 h-full object-cover" />
          <div className="px-5 py-8 w-2/3">
            ...
            <InfoSection header={"Date de naissance"} content={user.dateNaissance} />
            <Solde user={user}/>
          </div>
        </div>
